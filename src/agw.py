...
class GeneralizedMeanPooling(nn.Cell):
    ...
    def construct(self, x):
        x = ops.clip_by_value(x, clip_value_min=self.eps, clip_value_max=inf)
        x = self.pow(x, self.p)
...
def create_agw_net(num_class=1500, last_stride=1, pretrained=False, pretrained_dir='./model_utils', **kwargs):
...
class AGW_Loss(nn.LossBase):
    def __init__(
        self, ce=None, tri=None
    ):
        super().__init__()
        self.ce = ce
        self.tri = tri
...
class Debug_net(nn.Cell):
    def __init__(self, bnneck, last_conv_stride, num_classes):
        super().__init__()
        self.in_planes = 2048
        self.num_classes = num_classes
        ...
        if bnneck:
            ...
            self.classifier = nn.Dense(self.in_planes, self.num_classes, has_bias=True)
