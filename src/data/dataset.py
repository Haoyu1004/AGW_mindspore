...
class RandomIdentitySampler_2(ds.Sampler): # torch original
    """
    ...
    def __init__(self, data_source, batch_size, num_instances):
        ...
        for index, (_, pid) in enumerate(self.data_source):
            ...
            if isinstance(pid, np.ndarray):
                self.index_dic[pid.tolist()].append(index)
            elif isinstance(pid, int):
                self.index_dic[pid].append(index)
            else:
                raise TypeError("what the heck is the type of pid ?")
        self.pids = list(self.index_dic.keys())
...
def dataset_creator(
    ...
    if mode == 'train':
        ...
        if config.sampler == 'pk':
            sampler = RandomIdentitySampler_2(data_source=dataset_, 
                batch_size=config.batch_size_train, num_instances=config.num_instances)
                # rank_id=rank_id, rank_size=device_num)
        else:
        ...
        if isinstance(device_num, int) and device_num > 1:
            ...
            if config.sampler == 'pk':
                pass
                # data_set.source_len = len(sampler)    
        else:
            data_set = ds.GeneratorDataset(
                dataset_, ['img', 'pid'],
                sampler=sampler, num_parallel_workers=workers)
