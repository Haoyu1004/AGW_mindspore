...
class ResNetNL(nn.Cell):
    ...
    Backbone net
    """
    def __init__(self, pretrain="", non_layers=[0, 2, 3, 0], last_conv_stride=1):
        ...
        self.nonlocal_1_idx = sorted([self.layers[0] - (i + 1) for i in range(non_layers[0])])
        self.nonlocal_2 = nn.CellList(
            [NonLocal(512) for i in range(non_layers[1])])
        self.nonlocal_2_idx = sorted([self.layers[1] - (i + 1) for i in range(non_layers[1])])
        self.nonlocal_3 = nn.CellList(
            [NonLocal(1024) for i in range(non_layers[2])])
        self.nonlocal_3_idx = sorted([self.layers[2] - (i + 1) for i in range(non_layers[2])])
        self.nonlocal_4 = nn.CellList(
            [NonLocal(2048) for i in range(non_layers[3])])
        self.nonlocal_4_idx = sorted([self.layers[3] - (i + 1) for i in range(non_layers[3])])
        
        if len(self.nonlocal_1_idx) == 0 : self.nonlocal_1_idx = [-1]
        if len(self.nonlocal_2_idx) == 0 : self.nonlocal_2_idx = [-1]
        if len(self.nonlocal_3_idx) == 0 : self.nonlocal_3_idx = [-1]
        if len(self.nonlocal_4_idx) == 0 : self.nonlocal_4_idx = [-1]
