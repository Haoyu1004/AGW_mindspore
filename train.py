...
from src.utils.loss import OriTripletLoss, TripletLoss_WRT, CrossEntropyLoss
from src.agw import AGW_Loss, create_agw_net, Debug_net
...
def init_lr(num_batches):
    ...
    if config.lr_scheduler == 'single_step':
        lr = step_lr(config.lr, config.step_epoch, num_batches, config.max_epoch, config.gamma)
    elif config.lr_scheduler == 'multi_step':
        lr = multi_step_lr(config.lr, config.step_epoch, num_batches, config.max_epoch, config.gamma)
...
def get_callbacks(num_batches):
    ...
    ckpt_cb = ModelCheckpoint(prefix="agw-0321", directory=set_save_ckpt_dir(), config=config_ck)
    cb += [ckpt_cb]
...
def train_net():
    ...
    agwloss = AGW_Loss(
        ce=CrossEntropyLoss(num_classes=num_classes, label_smooth=config.label_smooth), 
        tri=TripletLoss_WRT() if config.wrt_loss else OriTripletLoss(batch_size=dataset1.get_batch_size())
    )
